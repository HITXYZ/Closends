...
class ZhihuSpider(SocialMediaSpider):
    ...
    def scrape_follows(self, user=None, number=0):
        ...
        response = requests.get(follows_url.format(user=user, include=follows_query, offset=0, limit=20),
                                headers=headers)
        if response.status_code == 404:  # 用户不存在或账号被封禁
            logging.warning('404 error. The user doesn\'t exist or has been blocked.')
            return []
    ...
    def scrape_followers(self, user=None, number=0):
        ...
        response = requests.get(followers_url.format(user=user, include=followers_query, offset=0, limit=20),
                                headers=headers)
        if response.status_code == 404:  # 用户不存在或账号被封禁
            logging.warning('404 error. The user doesn\'t exist or has been blocked.')
            return []
    ...
    def scrape_activities(self, user=None):
        ...
        for data in result.get('data'):
            ...
            if item.verb == 'QUESTION_FOLLOW' or item.verb == 'QUESTION_CREATE' or item.verb == 'MEMBER_VOTEUP_ARTICLE':
